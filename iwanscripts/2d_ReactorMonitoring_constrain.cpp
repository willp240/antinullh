...
LHFit_fit(const std::string &spectrum_phwr_unosc_filepath,
  ...
  */
  ...
  BinnedED data_set_pdf_e1("data_set_pdf_e1", axes_1d);
  data_set_pdf_e1.SetObservables(data_rep_1d);
  BinnedED data_set_pdf_e2("data_set_pdf_e2", axes_1d);
  data_set_pdf_e2.SetObservables(data_rep_1d);
  ...
  for (ULong64_t i = 0; i < n_reactors; i++){
	...
	if (apply_oscillation || is_further_reactors){
	  ...
	  if (j == 0)
	    data_set_pdf_e1.Add(*reactor_osc_data_1d_pdf[j][i]);
	  else if (j == 1)
	    data_set_pdf_e2.Add(*reactor_osc_data_1d_pdf[j][i]);
  ...
  */
  ...
  for (ULong64_t i = 0; i < n_reactors; i++){
	...
	nd_reactor_Int *= constraint_osc_mean;
	...
	Double_t min = 0.;
	Double_t max = 3.*nd_reactor_Int;
	if (min < 0) min = 0;
        minima[name] = min;
	maxima[name] = max;
	Double_t random = random_generator->Uniform(0.7,1.3);
	initial_val[name] = nd_reactor_Int*random;
	initial_err[name] = 0.5*nd_reactor_Int;

	lh_function.AddDist(*reactor_osc_nd_pdf[i]);
	if (known_reactor_constraints[i] > 0.){
	  Double_t sigma =  nd_reactor_Int*known_reactor_constraints[i];
	  lh_function.SetConstraint(name, nd_reactor_Int,sigma);
	  printf("  added CONSTRAINED reactor %d/%d: %s (min:%.3f max:%.3f) constraint: %.3f , sigma: %.3f data_int:%.0f\n", i+1, n_reactors, reactor_names[i].c_str(), min, max, nd_reactor_Int, sigma, data_set_pdf_integral);
	}
	else
	  printf("  added reactor %d/%d: %s (min:%.3f max:%.3f) constraint: %.3f , sigma: %.3f data_int:%.0f\n", i+1, n_reactors, reactor_names[i].c_str(), min, max, nd_reactor_Int, 0., data_set_pdf_integral);
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 5){
      std::cout<<"Error: 4(5) argument expected."<<std::endl;
...
const std::string &data_path = reactor_monitoring_folder+"sim_2d_data"+_dstream.str()+job_id_stream.str()+".root";
    ...
    thermal_powers.push_back(150.);//1000.);
    ...
    known_reactor_constraints.push_back(-1.);//0.06);
    known_reactor_constraints.push_back(-1.);//0.06);
    known_reactor_constraints.push_back(-1.);//0.06);
    known_reactor_constraints.push_back(-1.);//0.06);
    ...
    detector_names.push_back("2"); // Detector 2
    const size_t n_detectors = detector_names.size();

    std::vector<TVector3> detector_locations;
    //SK NK border
    //detector_locations.push_back(TVector3(38.077930, 127.005648, 0.)); //37.961175, 126.815154, 0.
    //detector_locations.push_back(TVector3(38.288457, 127.361670,0.)); //38.245568, 127.260614, 0.

    //NK China border
    detector_locations.push_back(TVector3(40.367669, 124.586077, 0.));//40.536125, 124.886173, 0.) );
    detector_locations.push_back(TVector3(40.522792, 124.905926, 0.));//40.234930, 124.417709, 0.) );
    ...
    //detector_positions.push_back(TVector3(0, 0, reactor_separation_d/2.)); // Detector 1
    //detector_positions.push_back(TVector3(0, 0, -1.*reactor_separation_d/2.)); // Detector 2
    ...
    */
    ...
    for (size_t i = 0; i < n_reactors; i++){
      ...
      TH1D h_reactor_total_power_uncert_d(reactor_power_uncert_strm.str().c_str(), reactor_power_uncert_strm.str().c_str(), 120, -0.3, 0.3);
