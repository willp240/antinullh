    ...
    with open(reactor_ratdb_filename, 'r') as ratdb_file:
        ...
        for line in ratdb_file:
            ...
            if 'type: "REACTOR",' in line:
                ...
                reactor_info[reactor_name]["cores"] = cores
                reactor_info[reactor_name]["core_powers"] = core_powers
                reactor_info[reactor_name]["latitudes"] = latitudes
                reactor_info[reactor_name]["longitudes"] = longitudes
                reactor_info[reactor_name]["core_types"] = core_types
    ...
    with open(reactor_ratdb_status_filename, 'r') as ratdb_status_file:
        ...
        for line in ratdb_status_file:
            ...
            if 'type: "REACTOR_STATUS",' in line:
                ...
                # get core scale information
                core_scales = (next_lines.split('core_power_scale_factor: ['))[1].split('],')[0]
                core_scales = core_scales.split(',')
                core_scales = map(str.strip, core_scales)
                core_scales = map(float, core_scales)
                
                ##################
                ## currently making error equal to value
                core_powers_err = core_scales
                ##################
                
                # write values to dictionary
                reactor_status_info[reactor_name] = {}
                reactor_status_info[reactor_name]["cores"] = cores
                reactor_status_info[reactor_name]["core_scales"] = core_scales
...
def get_reactor_info(reactor_ratdb_info, reactor_status_ratdb_info, reactor_list="All", position = "sno"):
    ...
    for reactor_name in reactor_ratdb_info:
        ...
        reactor_info[reactor_name]["core_scales"] = \
            reactor_status_ratdb_info[reactor_name]["core_scales"]
        reactor_info[reactor_name]["core_powers_err"] = reactor_status_ratdb_info[reactor_name]["core_powers_err"]

        #print "scales: ", reactor_info[reactor_name]["core_scales"], "powers: ", reactor_info[reactor_name]["core_powers"], "power errs: ", reactor_info[reactor_name]["core_powers_err"]
        # Now go through data and add some average values
        # rounded core power (and set as int)
        avg_core_power = 0
        if len(reactor_info[reactor_name]["core_powers"]) != len(reactor_info[reactor_name]["core_scales"]):
            print "\nUNEQUAL POWER AND SCALE LENGTHS FOR ", reactor_name, "    skipping.."
            continue
        if len(reactor_info[reactor_name]["core_powers"]) == 0:
            print "NO CORE POWER FOR ", reactor_name

        power_weights = []
        for i in range(len(reactor_info[reactor_name]["core_powers"])):
            core_power_i = reactor_info[reactor_name]["core_powers"][i]*reactor_info[reactor_name]["core_scales"][i]
            avg_core_power += core_power_i
            power_weights.append(avg_core_power)
        
        avg_core_power = int(round(avg_core_power/float(len(reactor_info[reactor_name]["core_powers"]))))
        reactor_info[reactor_name]["core_power"] = avg_core_power
...
def write_output_file(filename_output, reactor_info):
    ...
    with open(filename_output, 'ab+') as file_out:
        ...
        for key, values in reactor_info.items():
            #print key,str(values["distance"]),str(values["core_type"]),str(values["cores"]),str(values["core_power"]),str(values["core_power_err"])
            file_out.write(key\
                           +','+str(values["distance"])\
