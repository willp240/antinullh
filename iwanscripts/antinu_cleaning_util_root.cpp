...
void process_cuts(const std::string filename_input, const std::string filename_output, Double_t energy_ep_min, Double_t energy_ep_max, Double_t energy_n_min, Double_t energy_n_max,Double_t deltaTmin, Double_t deltaTmax, Double_t promptRmax, Double_t lateRmax, Double_t deltaRmax) {
  ...
  double delR = 6000;
  for (double r0 = 0; r0 < 5100; r0 = r0 + delR){
    num_radii += 1;
    radii.push_back(r0);
  }

  std::cout<<"number of radii bins: "<<num_radii<<std::endl;
  double del_pos_min = -2000.;
  double del_pos_max = 2000.;
  size_t del_pos_bins = 400;

  TH1D *h_delposXs [num_radii];
  TH1D *h_delposYs [num_radii];
  TH1D *h_delposZs [num_radii];

  for (size_t i = 0; i < num_radii; i++){
    h_delposXs[i] = new TH1D("delX", "delX",del_pos_bins, del_pos_min, del_pos_max);
    h_delposYs[i] = new TH1D("delY", "delY",del_pos_bins, del_pos_min, del_pos_max);
    h_delposZs[i] = new TH1D("delZ", "delZ",del_pos_bins, del_pos_min, del_pos_max);
    std::stringstream titlestrmX;
    titlestrmX<<"delX_R"<<radii[i];
    std::stringstream titlestrmY;
    titlestrmY<<"delY_R"<<radii[i];
    std::stringstream titlestrmZ;
    titlestrmZ<<"delZ_R"<<radii[i];
    h_delposXs[i]->SetTitle(titlestrmX.str().c_str());
    h_delposXs[i]->SetName(titlestrmX.str().c_str());
    h_delposYs[i]->SetTitle(titlestrmY.str().c_str());
    h_delposYs[i]->SetName(titlestrmY.str().c_str());
    h_delposZs[i]->SetTitle(titlestrmZ.str().c_str());
    h_delposZs[i]->SetName(titlestrmZ.str().c_str());
  }
  
  std::string fittername = "scintFitter";
  //std::string fittername = "positionTimeLikelihood";
  ...
  for(size_t ie = 0; abs(ie) < abs(n_entries); ie++){  
    ...
    TVector3 mc_vecR = rmcparticle1.GetPosition();
	...
	bool BadFit2 = true;
	std::vector<std::string> names = rEV2.GetFitNames();//GetFitResults().GetNames();
	for (size_t boo = 0; boo < names.size(); boo++)
	  std::cout<<names[boo]<<std::endl;
	if( rEV2.FitResultExists(fittername)) {
	  if( rEV2.GetFitResult(fittername).GetVertex(0).ContainsPosition() ) {
	    if( rEV2.GetFitResult(fittername).GetVertex(0).ValidPosition() ) {
	      if( rEV2.GetFitResult(fittername).GetVertex(0).ContainsEnergy() ) {
		if( rEV2.GetFitResult(fittername).GetVertex(0).ValidEnergy() ) {
		  
		  BadFit2 = false;
		  nextFitted = 1;
		  RAT::DS::FitResult fResult = rEV2.GetFitResult(fittername);
	...
	if( rEV1.FitResultExists(fittername)) {
	  if( rEV1.GetFitResult(fittername).GetVertex(0).ContainsPosition() ) {
	    if( rEV1.GetFitResult(fittername).GetVertex(0).ValidPosition() ) {
	      if( rEV1.GetFitResult(fittername).GetVertex(0).ContainsEnergy() ) {
		if( rEV1.GetFitResult(fittername).GetVertex(0).ValidEnergy() ) {
		  BadFit1 = false;
		  Fitted = 1;
		  RAT::DS::FitResult fResult = rEV1.GetFitResult(fittername);
		  RAT::DS::FitVertex fVertex = fResult.GetVertex(0);
	...
	std::cout<<"here1"<<" fit1: "<<Fitted<<" fit2: "<<nextFitted<<std::endl;
		...
		if (deltaR < deltaRmax){
		  if (energy_ep_min <= ev_energy && ev_energy <= energy_ep_max){
			...
			for( size_t iPMT = 0; iPMT < calibratedPMTs.GetCount(); iPMT++ ){
		      ...
		      size_t delR_i = (size_t)floor(mc_vecR.Mag()/delR);
		      //std::cout<<"delR_i: "<<delR_i<<std::endl;
		      
		      h_delposXs[delR_i]->Fill(ev_vecR.X() - mc_vecR.X());
		      h_delposYs[delR_i]->Fill(ev_vecR.Y() - mc_vecR.Y());
		      h_delposZs[delR_i]->Fill(ev_vecR.Z() - mc_vecR.Z());
