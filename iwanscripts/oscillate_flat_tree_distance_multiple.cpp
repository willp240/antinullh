...
Int_t main(int argc, char* argv[]){
  if (argc != 11)
    std::cout<<"10 arguments expected!"<<std::endl;
  else{
    const std::string root_in = argv[1];
    const std::string root_out = argv[2];
    const double distance = atof(argv[6]);
    const size_t pass_min = atoi(argv[7]);
    const size_t pass_max = atoi(argv[8]);
    const std::string cut_label = argv[9];
    const std::string osc_label = argv[10];
    ...
    for (size_t pass_i = pass_min; pass_i <= pass_max; pass_i++){
        ...
        write_file(root_in_i, root_out_i, del_m_sqr_21, sin_sqr_theta_12, sin_sqr_theta_13, distance);
    }
  }
}
