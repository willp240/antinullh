...
void Po212alphaFit(){
    ...
    Tc->SetBranchAddress("Po_parentpdg1", &parentpdg1);
    TNtuple* nt = new TNtuple("nt","nt","delayedEcorr"); 
    std::string filepath =  "/data/snoplus3/weiii/BiPo212/fullFill/rat-7.0.8/SimNtuple/Bipo212/";
    ...
    */
    ...
    // Construct Gaus PDF
    /*
    RooRealVar mu("mu", "mean parameter", 0.7, 1.2, 1.5);
    RooRealVar sigma("sigma", "width parameter", 0.1, 0.05, 1.0);

    RooGaussian gaus("gaus", "Gaussian PDF", delayedEcorr, mu, sigma); 
    gaus.fitTo(data);
    */
    RooKeysPdf k_212("k_212","k_212",delayedEcorr,data,RooKeysPdf::NoMirror) ;

    RooPlot* frame = delayedEcorr.frame();
    data.plotOn(frame,Name("data"),Binning(38)) ;
    //gaus.plotOn(frame,Name("gaus"),Binning(14)) ;
    k_212.plotOn(frame,Name("k_212"),Binning(38)) ;
    frame->Draw("E0");
    ...
    leg1->AddEntry(frame->findObject("k_212"),"212 alpha KDE", "L");
    leg1->Draw("same");  
   ...
   w->writeToFile("Po212alphaFit.root");
