...
bool IsDelayEv(int run, bool&partialFit, double&posX, double&posY, double&posZ,double delayR, double&posZAfterAVoffset, int& nhits,int& nhitsCleaned,double&energy,ULong64_t&clock50, ULong64_t&DCapplied,ULong64_t&DCflagged, Int_t&triggerWord, bool is_data);
bool IsPromptEv(int run,bool&partialFit, double&posX, double&posY, double&posZ, double promptR, double&posZAfterAVoffset,int& nhits,int& nhitsCleaned,double&energy,double&dR, ULong64_t&DCapplied,ULong64_t&DCflagged, Int_t&triggerWord,  bool is_data);
...
bool IsDelayEv(int run, bool&partialFit, double&posX, double&posY, double&posZ,double delayR, double&posZAfterAVoffset, int& nhits,int& nhitsCleaned,double&energy,ULong64_t&clock50, ULong64_t&DCapplied,ULong64_t&DCflagged, Int_t&triggerWord, bool is_data){
    ...
    if(!partialFit ) return false;
    
    // FV check    
    if( delayR > 5000.0 ) return false;
    //std::cout<< "R: "<<R <<std::endl;
    // apply DC mask
    if (is_data == 1){
        if (((DCapplied & 0x210000000242) & DCflagged ) != (DCapplied & 0x210000000242)) return false;
    }
    if(posZAfterAVoffset < 850.)  return false;

    if((static_cast<double>(nhitsCleaned)/static_cast<double>(nhits) <= 0.8)) return false;

    if(energy < 0.6 || energy > 2.5)  return false;
    std::cout<<"Delay pass energy cuts"<<std::endl;
    return true;
}
bool IsPromptEv(int run,bool&partialFit, double&posX, double&posY, double&posZ, double promptR, double&posZAfterAVoffset,int& nhits, int& nhitsCleaned,double&energy,double&dR, ULong64_t&DCapplied,ULong64_t&DCflagged, Int_t&triggerWord,  bool is_data){
   ...
   // std::cout<< "pass the dR cuts"<<std::endl;
    ...
    std::cout<<"Pass DCapplied"<<std::endl;

    if( promptR > 5000.0 ) return false;
    if(posZAfterAVoffset < 850.)  return false;
    std::cout<< "Bi pass Z"<<std::endl;

    if((static_cast<double>(nhitsCleaned)/static_cast<double>(nhits) <= 0.8)) return false;
    std::cout<< "Bi pass the ratio"<<std::endl;
...
void RnPo215Recon(std::string inputpath, int RUN_NUM, std::string output_root_address, std::string Proc_rat, bool is_data){// detector_status: 0 is water phase, 1 is partial fill, 2 is  fulfill
    ...
    int nhits; int nhitsCleaned; 
    bool scintFit, partialFit;
    ULong64_t clock50;
    ULong64_t DCapplied;
    ULong64_t DCflagged;
    Int_t triggerWord;
    ...
    PoT->Branch("Po_skyShine", &skyShine);
    ...
    BiT->Branch("Bi_skyShine", &skyShine);
    ...
    Tc->SetBranchAddress("skyShine", &skyShine);
    ...
    for (int iEntry = 0; iEntry < Tc->GetEntries(); iEntry++){
        ...
        Tc->GetEntry(iEntry);
        if(skyShine<1.) continue;
        
        
        delayposZAfterAVoffset = posZ - ZAVoffset;
        delayR = pow(( posX*posX + posY*posY + delayposZAfterAVoffset*delayposZAfterAVoffset),0.5 );

        //std::cout<<"IsDelayEv "<<IsDelayEv(partialFit, posX, posY, posZ, posZAfterAVoffset, energy, clock50,DCapplied,DCflagged,triggerWord)<<std::endl;
        if (!IsDelayEv(RUN_NUM,partialFit, posX, posY, posZ, delayR,delayposZAfterAVoffset, nhits, nhitsCleaned,energy, clock50,DCapplied,DCflagged,triggerWord, is_data)){
            continue;
        }
        ...
        for (int jEntry = iEntry - 1; jEntry >= 0; jEntry --){
            ...
            //std::cout<<jEntry<<" IsPromptEv: "<<IsPromptEv(partialFit, posX, posY, posZ, posZAfterAVoffset,energy,dR, DCapplied,DCflagged, triggerWord)<<std::endl;
            if (!IsPromptEv(RUN_NUM,partialFit, posX, posY, posZ, promptR,promptposZAfterAVoffset,nhits, nhitsCleaned,energy,dR, DCapplied,DCflagged, triggerWord, is_data)){
                continue;
            }
